import { createLocalHistoryStorage, historyPath, type LocalHistoryStorage, walkFiles } from './local-history';
...
export const codexSessionsDir = (storage: LocalHistoryStorage) => historyPath(storage, '.codex', 'sessions');

export const hasCodexHistory = (storage: LocalHistoryStorage) => storage.exists(codexSessionsDir(storage));

export const listCodexSessionFiles = (storage: LocalHistoryStorage) =>
  walkFiles(storage, codexSessionsDir(storage), (fileName) => fileName.endsWith('.jsonl'));

export const readCodexThreadNames = (storage: LocalHistoryStorage): Map<string, string> => {
  const names = new Map<string, string>();
  const indexPath = historyPath(storage, '.codex', 'session_index.jsonl');
  if (!storage.exists(indexPath)) return names;

  for (const line of storage.readText(indexPath).split('\n')) {
...
export const readCodexSessions = (storage = createLocalHistoryStorage()): CodexSession[] => {
  const sessions: CodexSession[] = [];

  for (const filePath of listCodexSessionFiles(storage)) {
...
export const findLatestCodexRateLimits = (
  storage = createLocalHistoryStorage(),
  recentFileLimit = 40,
): CodexRateLimitSnapshot | null => {
  let latest: CodexRateLimitSnapshot | null = null;
  const files = listCodexSessionFiles(storage).sort();
