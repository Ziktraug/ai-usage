# Plan 031: Distill six proven decisions into ADRs

> **Status: BLOCKED** until the parallel session-analysis work is merged into main, the portfolio worktree is recreated/rebased, cited code is re-read, and `<NEW_MAIN_SHA>` is replaced. Then set TODO. Execute sequentially after plan 030.
>
...
- **In scope**: exactly six ADRs: URL state; loopback no-LAN; file Sync; exact revisions; fixed window; two-tier design system.
...
3. Run the slice-specific gates: each ADR has Context Decision Consequences Rejected alternatives Evidence; >=2 primary citations; no invented rationale. **Expected**: every named invariant is machine-checked and passes.
...
Use existing colocated Bun model tests and role-based Playwright tests as structural patterns. Add deterministic synthetic fixtures only. Required oracle: each ADR has Context Decision Consequences Rejected alternatives Evidence; >=2 primary citations; no invented rationale.
