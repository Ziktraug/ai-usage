...
- 2026-07-16 — Step 4 complete: added the scoped Effect control-plane service/layer with a bounded queue, one worker by default, atomic `SubscriptionRef` state, `FiberMap` cadence timers, policy revisions, manual commands, timeouts, RTK dependency watermarks, publication dedupe, and scoped interruption. Ten deterministic `TestClock`/in-memory tests cover startup, cadence, commands, policy races, failures, dormancy, one/multi-worker publication, timeout, and shutdown. Workspace typechecking, package-boundary/export checks, Ultracite, and `git diff --check` passed.
- 2026-07-16 — Step 5 complete: added a timer-free one-shot executor that shares source adapters and home policy, routed normal reports and snapshots through selected local sources without provider communication, routed `quota` exclusively through `codex.usage-limits`, and split CLI publication onto durable stored rows, normalized Cursor attribution, and stored provider observations. Process tests prove the normal report does not launch a provider command, paused policy is explicit, and disabled-source history remains visible. All 107 CLI/report-data tests, workspace typechecking, Ultracite, the complete workspace test suite, and `git diff --check` passed.
- 2026-07-16 — Step 6 complete: migrated Nitro production to its official Bun preset and pinned Bun start path, installed one injectable process-owned `ManagedRuntime` with direct adapters/SQLite and stored-only exact-revision publication, and wired project/merge invalidations to the publication queue when the runtime is active. Because the Bun preset closes through `srvx` without invoking Nitro's application `close` hook, the plugin owns bounded `SIGINT`/`SIGTERM` disposal in addition to the Nitro hook; the production smoke proves the scoped runtime stops before listener exit and the port is reusable. Seventy-five server tests cover runtime singleton startup, timeout, direct SQLite collection, interruption, and idempotent disposal. Workspace typechecking, Ultracite, Bun production build/smoke, a seeded 1.25-second responsiveness watchdog, and all five production E2E tests passed.
