# Plan 032: Prepare the frontend showcase and publication material

> **Status: BLOCKED** until the parallel session-analysis work is merged into main, the portfolio worktree is recreated/rebased, cited code is re-read, and `<NEW_MAIN_SHA>` is replaced. Then set TODO. Execute sequentially after plan 031.
>
...
- **In scope**: one deterministic 1600x900 composite hero showing Overview plus session detail; README; case study; publication checklist; no gh edit.
...
3. Run the slice-specific gates: synthetic image <1MiB; deterministic dimensions/pixels; claims trace to tests/ADRs; full gates. **Expected**: every named invariant is machine-checked and passes.
...
Use existing colocated Bun model tests and role-based Playwright tests as structural patterns. Add deterministic synthetic fixtures only. Required oracle: synthetic image <1MiB; deterministic dimensions/pixels; claims trace to tests/ADRs; full gates.
