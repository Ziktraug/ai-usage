...
export const MAX_USAGE_SNAPSHOT_BYTES = 64 * 1024 * 1024;
...
const readBoundedSnapshotBytes = async (filePath: string, openFile: OpenUsageSnapshotFile): Promise<Uint8Array> => {
  ...
  try {
    ...
    if (!fileStat.isFile() || fileStat.size > MAX_USAGE_SNAPSHOT_BYTES) {
      throw new Error('snapshot input must be a bounded regular file');
    }

    const chunks: Uint8Array[] = [];
    let totalBytes = 0;
    while (totalBytes <= MAX_USAGE_SNAPSHOT_BYTES) {
      const remainingBytes = MAX_USAGE_SNAPSHOT_BYTES + 1 - totalBytes;
    ...
    if (totalBytes > MAX_USAGE_SNAPSHOT_BYTES) {
