...
import { productionBuildLockPath, runProductionWebBuild, withProductionBuildLock } from './vite-production-build';
...
describe('production web build command', () => {
  test('cannot be bypassed by a Turbo cache restore from the root build entrypoint', async () => {
    ...
    expect(packageManifest.scripts?.build).toBe('bun --no-env-file vite-production-build.ts');
  });
  ...
  test('runs every build phase under the lock and cleans only production output', async () => {
    ...
    const devOutputFile = path.join(webDirectory, '.output-dev', 'nitro', 'dev.txt');
    const productionOutputFiles = [
      path.join(webDirectory, '.output-build', 'nitro', 'old-server.txt'),
      path.join(webDirectory, '.output-build', 'vite', 'old-cache.txt'),
      path.join(webDirectory, '.output-build', 'work', 'old-work.txt'),
      path.join(webDirectory, 'dist', 'old-client.txt'),
    ];
    await Promise.all(
