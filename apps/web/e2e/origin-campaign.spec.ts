...
test('makes the neutral origin default and keeps singleton campaigns unqualified', async ({ page }) => {
  ...
  // The open panel covers the row's right side; its first cell stays visible beside the panel.
  await page.locator('[data-session-row-id]').filter({ hasText: 'Build report UI' }).locator('td').first().click();
  ...
  // The qualifier must describe the whole campaign, not just what the Members tab lists.
  await expect(campaignScope).toHaveAttribute(
    'title',
    "Values here cover the whole campaign: every session matching the current filters plus its rolled-up automated reviews, including any not listed under Members. Rounds and Timeline read the root session's local history.",
  );
  await drawer.getByRole('tab', { name: 'Summary' }).click();
  await expect(drawer.locator('[data-detail-item="Subagent"]')).toContainText('No');
  // The campaign header and the stat strip state one aggregation, not two.
