...
const sessionCampaignExportRow = (row: SessionPresentationRow): SessionCampaignExportRow => {
  ...
  return {
    ...
    costApprox: row.costApprox ?? null,
    costKnown: row.costKnown,
    costQuota: row.costQuota ?? null,
    ...
    rtkCommandCount: row.rtkCommandCount ?? null,
    rtkInputTokens: row.rtkInputTokens ?? null,
    rtkOutputTokens: row.rtkOutputTokens ?? null,
    rtkSavedTokens: row.rtkSavedTokens ?? null,
    tokCr: row.tokCr,
    tokCw: row.tokCw,
    tokIn: row.tokIn,
    tokOut: row.tokOut,
