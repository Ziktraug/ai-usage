<script lang="ts">
  ...
  const editorSection = css({
    display: 'grid',
    gridTemplateAreas: { base: '"header" "actions" "editor"', md: '"header actions" "editor editor"' },
  ...
  const documentToolbar = css({
    ...
    borderRight: { base: '1px solid token(colors.line)', md: 0 },
    bg: 'surfaceMuted',
    minW: 0,
    '& > span': { h: 'auto', minH: '22px', whiteSpace: 'normal', overflowWrap: 'anywhere', lineHeight: 1.5 },
  });
  const documentActions = css({
    ...
    p: '10px 12px',
    border: '1px solid token(colors.line)',
    borderTop: { base: 0, md: '1px solid token(colors.line)' },
    borderLeft: { base: '1px solid token(colors.line)', md: 0 },
    borderTopRightRadius: { base: 0, md: 'sm' },
    bg: 'surfaceMuted',
    '& > button': { minH: { base: '44px', md: '32px' }, px: '9px', fontSize: '11px' },
  });
  const editorArea = css({
    gridArea: 'editor',
    boxSizing: 'border-box',
    minH: { base: '45vh', md: 'clamp(320px, 48vh, 720px)' },
    maxW: '100%',
    w: '100%',
    overflowX: 'hidden',
