...
describe('P2 Overview/range client dependency closure', () => {
  test('traverses Svelte leaves, aliases, workspace exports, and barrels without Solid, server, Node, or writers', () => {
    const closure = closureFrom(entryPaths, realOptions);

    expect(closureViolations(closure, repositoryRoot)).toEqual([]);
    expect(closure.files.has(join(featureDirectory, 'activity-timeline.svelte'))).toBe(true);
    expect(closure.files.has(join(featureDirectory, 'timeline-model.ts'))).toBe(true);
    expect(closure.files.has(join(rangeDirectory, 'report-range-control.svelte'))).toBe(true);
    expect(closure.files.has(join(rangeDirectory, 'report-range-model.ts'))).toBe(true);
    expect(closure.files.has(join(repositoryRoot, 'packages/report-core/src/focused-report-query.ts'))).toBe(true);
  ...
  test('detects hidden .svelte.ts, package-barrel, $lib alias, server, writer, and Solid leaks synthetically', () => {
