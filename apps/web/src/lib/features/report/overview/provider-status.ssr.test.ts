...
const optionalMachineId = (
  machineId: string | null | undefined,
  machineContext: string | null | undefined,
): { machineId?: string } => {
  if (machineId === null) {
    return {};
  }
  const resolvedId = machineId ?? machineContext;
  return resolvedId ? { machineId: resolvedId } : {};
};
...
const providerView = (input: {
  ...
  /** Defaults to the label. Pass `null` to render a labelled observation without an id. */
  ...
  provider: {
    ...
    ...optionalMachineId(input.machineId, input.machineContext),
...
describe('rendered provider status panel', () => {
  ...
  test('renders labelled observations separately when their machine ids are absent', () => {
    const html = renderPanel([
      providerView({
        key: 'cursor',
        label: 'Cursor',
    ...
    const renderedLines = [...html.matchAll(NO_QUOTA_LINE_PATTERN)].map(([, text]) => (text ?? '').trim());
    expect(renderedLines).toEqual(['Fixture Laptop · Cursor — partial', 'Fixture Workstation · Claude — unsupported']);
  });
