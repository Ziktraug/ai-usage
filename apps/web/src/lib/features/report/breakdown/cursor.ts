...
/**
 * Resolves each complete commit group before applying the inclusive bounds.
 * This prevents one branch row's scoring fallback from pulling a commit into a
 * period when another row carries the authoritative commit date. Every row in
 * an included group survives so grouping cannot lose branch or machine data.
 * Groups with no usable date are kept — a filter default never excludes
 * "unknown".
 */
export const cursorRowsInRange = <Row extends CursorCommitAttributionFacet>(
  rows: readonly Row[],
  ...
  const activityByGroup = new Map<string, CursorCommitActivity>();
  for (const row of rows) {
    const key = cursorCommitGroupKey(row, cursorCommitMetricSignature(row));
    const activity = cursorCommitActivity(row);
    const current = activityByGroup.get(key) ?? { source: 'none', time: null };
    if (outranksCursorActivity(current, activity)) {
      activityByGroup.set(key, activity);
    } else if (!activityByGroup.has(key)) {
      activityByGroup.set(key, current);
    }
  }
  const withinRange = (row: Row): boolean => {
    const key = cursorCommitGroupKey(row, cursorCommitMetricSignature(row));
    const time = activityByGroup.get(key)?.time ?? null;
    if (time === null) {
      return true;
    }
...
const outranksCursorActivity = (current: CursorCommitActivity, activity: CursorCommitActivity): boolean => {
  const currentRank = cursorDateSourceRank[current.source];
  const activityRank = cursorDateSourceRank[activity.source];
  if (activityRank !== currentRank) {
    return activityRank > currentRank;
  }
  return activity.time !== null && current.time !== null && activity.time < current.time;
};
...
export const groupCursorCommits = (rows: readonly CursorCommitAttributionFacet[]): CursorCommitGroup[] => {
  ...
  for (const row of rows) {
    ...
    const activity = cursorCommitActivity(row);
    if (outranksCursorActivity({ source: draft.source, time: draft.time }, activity)) {
