...
const callCapability = async <T>(
  ...
  try {
    ...
    return unwrap(await call(capability, options));
  } catch (error) {
...
export const createSkillsRouter = (selectCapability: SelectSkillsCapability) => {
  const skills = implement(skillsContract);

  return {
    createTargetDirectory: skills.createTargetDirectory.handler(({ input, signal }) =>
      callCapability(selectCapability, signal, (capability, options) =>
        capability.createTargetDirectory(input, options),
      ),
    ),
    projectInventories: skills.projectInventories.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.readProjectInventories(options)),
    ),
    knownProjectPaths: skills.knownProjectPaths.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.readKnownProjectPaths(options)),
    ),
    ...
    previewReconcileAll: skills.previewReconcileAll.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.previewReconcileAll(options)),
    ),
    projectMarkdown: skills.projectMarkdown.handler(({ input, signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.readProjectMarkdown(input, options)),
    ),
    reconcileAll: skills.reconcileAll.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.reconcileAll(options)),
    ),
    ...
    refreshSnapshot: skills.refreshSnapshot.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.refreshSnapshot(options)),
    ),
    saveConfig: skills.saveConfig.handler(({ input, signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.saveConfig(input, options)),
    ),
    saveManagedMarkdown: skills.saveManagedMarkdown.handler(({ input, signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.saveMarkdown(input, options)),
    ),
    snapshot: skills.snapshot.handler(({ signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.readSnapshot(options)),
    ),
    toggleProjection: skills.toggleProjection.handler(({ input, signal }) =>
      callCapability(selectCapability, signal, (capability, options) => capability.toggleSkill(input, options)),
    ),
  };
};
