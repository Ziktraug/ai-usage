...
interface MachineGroup {
  /** Grouping and rendered identity, namespaced so real machine ids cannot collide with fallbacks. */
  readonly id: string;
  readonly views: ProviderStatusView[];
}

const machineLabelFor = (views: readonly ProviderStatusView[]): string | null =>
  [...new Set(views.map((view) => view.machineContext).filter((label): label is string => label !== null))].sort(
...
const compareMachineGroups = (left: MachineGroup, right: MachineGroup): number => {
  const leftLabel = machineLabelFor(left.views);
  const rightLabel = machineLabelFor(right.views);
  if (leftLabel === null || rightLabel === null) {
    if (leftLabel === rightLabel) {
      return left.id.localeCompare(right.id);
    }
    return leftLabel === null ? 1 : -1;
  }
  const byLabel = leftLabel.localeCompare(rightLabel);
  return byLabel === 0 ? left.id.localeCompare(right.id) : byLabel;
};

const machineGroupIdentity = (view: ProviderStatusView, index: number): string => {
  const machineId = view.provider.machineId;
  if (machineId !== undefined) {
    return `machine:${machineId}`;
  }
  if (view.machineContext === null) {
    return 'scope:unscoped';
  }
  // A display label is not an identity. Keep a labelled observation separate when its producer did
  // not supply an id; otherwise two unrelated machines collapse merely because their labels match
  // (and even different labels used to collapse under the same empty key).
  return `observation:unidentified:${index}:${view.provider.key}`;
};
...
export const providerMachineLines = (providers: readonly ProviderStatusView[]): ProviderMachineLine[] => {
  ...
  for (const [index, view] of providers.entries()) {
    const id = machineGroupIdentity(view, index);
    const group = byMachine.get(id) ?? { id, views: [] };
    group.views.push(view);
    byMachine.set(id, group);
  }
  return [...byMachine.values()].sort(compareMachineGroups).map(({ id, views }) => {
    const label = machineLabelFor(views);
