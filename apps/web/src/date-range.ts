...
export const dateBoundsForRange = (
  ...
  if (mode === 'today') {
    return { from: startOfDay(generatedAt), to: endOfDay(generatedAt) };
  }
  if (mode === '7d') {
    return { from: rollingDaysAgo(generatedAt, 7), to: null };
  }
  if (mode === '30d') {
...
export const rowMatchesDateBounds = (row: SerializedRow, bounds: DateBounds) => {
  ...
  if (bounds.from && time < bounds.from.getTime()) {
    return false;
  }
  if (bounds.to && time > bounds.to.getTime()) {
    return false;
  }
  return true;
};
