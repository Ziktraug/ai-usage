...
import { Deferred, Duration, Effect, Fiber, Layer, Ref, TestClock, TestContext } from 'effect';
...
describe('source control plane', () => {
  test('keeps a gated source queued until the writer permit is available', async () => {
    const program = Effect.scoped(
      Effect.gen(function* () {
        ...
        const sourceStarted = yield* Ref.make(false);
        const writerGate = createUsageEngineWriterGate();
        yield* Effect.forkScoped(
          writerGate.withEffect(
            Ref.update(events, (current) => [...current, 'holder-enter']).pipe(
              Effect.andThen(Deferred.succeed(holderEntered, undefined)),
              Effect.andThen(Deferred.await(releaseHolder)),
              Effect.andThen(Ref.update(events, (current) => [...current, 'holder-exit'])),
            ),
          ),
        );
        ...
        const control = yield* createSourceControl({
          ...
          sourceTimeout: Duration.seconds(1),
          sources: new Map([
            [
              'claude.sessions',
              fakeSource('claude.sessions', () =>
                Ref.set(sourceStarted, true).pipe(
                  Effect.andThen(Ref.update(events, (current) => [...current, 'source-run'])),
                  Effect.as(successResult(true)),
                ),
              ),
            ],
          ]),
        ...
        const queued = yield* waitFor(control, (view) => sourceView(view, 'claude.sessions').lifecycle === 'queued');
        yield* TestClock.adjust(Duration.seconds(2));
        expect(sourceView(queued, 'claude.sessions').lifecycle).toBe('queued');
        expect(yield* Ref.get(sourceStarted)).toBe(false);
        ...
        yield* waitFor(
          ...
          (view) =>
            ...
            !view.publication.running,
        );
        expect(yield* Ref.get(events)).toEqual(['holder-enter', 'holder-exit', 'source-run', 'publish:1']);
      }),
    );
  ...
  test('cancels queued bootstrap collection when autonomous admission closes', async () => {
    const program = Effect.scoped(
      Effect.gen(function* () {
        const events = yield* Ref.make<string[]>([]);
        const releaseHolder = yield* Deferred.make<void>();
        ...
        yield* Effect.forkScoped(
          writerGate.withEffect(
            Deferred.succeed(holderEntered, undefined).pipe(Effect.andThen(Deferred.await(releaseHolder))),
          ),
        );
        yield* Deferred.await(holderEntered);
        const { store } = yield* makePolicyStore();
        ...
        const control = yield* createSourceControl({
          ...
          sources: new Map([
            [
              'claude.sessions',
              fakeSource('claude.sessions', () =>
                Ref.update(sourceRuns, (count) => count + 1).pipe(Effect.as(successResult(true))),
              ),
            ],
          ]),
          writerGate,
        });
        yield* waitFor(control, (view) => sourceView(view, 'claude.sessions').lifecycle === 'queued');

        yield* control.stopAutonomousCollection;
        yield* Deferred.succeed(releaseHolder, undefined);
        yield* waitFor(
          control,
          (view) =>
            sourceView(view, 'claude.sessions').lifecycle !== 'queued' &&
            view.queueDepth === 0 &&
            view.runningCount === 0 &&
            view.publication.revision !== undefined &&
            !view.publication.running,
        ...
        expect(yield* Ref.get(sourceRuns)).toBe(0);
      }),
    );
  ...
  test('stops autonomous cadence admission while preserving explicit command work', async () => {
    const program = Effect.scoped(
      Effect.gen(function* () {
        const events = yield* Ref.make<string[]>([]);
        const { store } = yield* makePolicyStore();
        ...
        yield* waitFor(control, () => Effect.runSync(Ref.get(runs)) === 1);
  ...
  test('shows pausing immediately while a gated source drains before the policy write', async () => {
    const program = Effect.scoped(
      Effect.gen(function* () {
        const events = yield* Ref.make<string[]>([]);
        const started = yield* Deferred.make<void>();
        const release = yield* Deferred.make<void>();
        const runs = yield* Ref.make(0);
        const policyWrites = yield* Ref.make(0);
        const policies = yield* Ref.make<SourcePolicyOverrides>({});
        const writerGate = createUsageEngineWriterGate();
        const source = fakeSource('claude.sessions', () =>
          Effect.gen(function* () {
            yield* Ref.update(runs, (count) => count + 1);
            yield* Ref.update(events, (current) => [...current, 'source-start']);
            yield* Deferred.succeed(started, undefined);
            yield* Deferred.await(release);
            yield* Ref.update(events, (current) => [...current, 'source-exit']);
            return successResult(true);
          }),
        );
        const publication = yield* makePublication(events);
        const control = yield* createSourceControl({
          policyStore: {
            load: Ref.get(policies),
            setEnabled: (sourceId, enabled) =>
              Ref.update(policyWrites, (count) => count + 1).pipe(
                Effect.andThen(Ref.update(events, (current) => [...current, 'policy-write'])),
                Effect.andThen(
                  Ref.update(policies, (current) => updateSourcePolicyOverrides(current, sourceId, enabled) ?? {}),
                ),
              ),
        ...
        const disableFiber = yield* Effect.fork(control.setEnabled('claude.sessions', false));
        const pausing = yield* waitFor(control, (view) => sourceView(view, 'claude.sessions').lifecycle === 'pausing');

        expect(sourceView(pausing, 'claude.sessions').policy).toBe('disabled');
        expect(yield* Ref.get(policyWrites)).toBe(0);
        yield* Deferred.succeed(release, undefined);
        ...
        expect(sourceView(stopped, 'claude.sessions').policy).toBe('disabled');
        expect(yield* Ref.get(events)).toEqual(['source-start', 'source-exit', 'policy-write', 'publish:1']);
        yield* TestClock.adjust(Duration.minutes(5));
        expect(yield* Ref.get(runs)).toBe(1);
      }),
    );
  ...
  test('rolls back optimistic source policy when persistence fails', async () => {
    const program = Effect.scoped(
      Effect.gen(function* () {
        const events = yield* Ref.make<string[]>([]);
        const publication = yield* makePublication(events);
        ...
        const control = yield* createSourceControl({
          policyStore: {
            load: Effect.succeed({}),
            setEnabled: () => Effect.fail(new Error('private policy failure')),
          },
          publication: publication.port,
          sources: new Map([
            [
              'claude.sessions',
              fakeSource('claude.sessions', () =>
                Ref.updateAndGet(runs, (count) => count + 1).pipe(Effect.as(successResult(false))),
              ),
            ],
          ]),
        });
        const initiallyScheduled = yield* waitFor(
          control,
          (view) =>
            sourceView(view, 'claude.sessions').lastOutcome === 'success' && view.publication.revision === 'revision-1',
        );
        const originalDueAt = sourceView(initiallyScheduled, 'claude.sessions').nextDueAt;
        yield* TestClock.adjust(Duration.seconds(30));

        const error = yield* control.setEnabled('claude.sessions', false).pipe(Effect.flip);
        expect(error.reason).toBe('policy-write-failed');
        const rolledBack = yield* control.getSnapshot;
        expect(sourceView(rolledBack, 'claude.sessions')).toMatchObject({
          lifecycle: 'scheduled',
  ...
  test('does not lose a producer change while RTK is already running with multiple workers', async () => {
    ...
    const runEvents = boundaryEvents(capture.events(), 'source.run');
    const enrichmentEvents = boundaryEvents(capture.events(), 'enrichment');
    expect(runEvents).toHaveLength(3);
    expect(enrichmentEvents).toHaveLength(2);
    expect(enrichmentEvents.every((event) => event.annotations.sourceId === 'rtk.savings')).toBe(true);
    expect(new Set(runEvents.map((event) => event.eventId)).size).toBe(3);
    for (const event of runEvents) {
      if (event.annotations.sourceId === 'rtk.savings') {
        expect(event.services).toHaveLength(0);
      } else {
