...
export const cancelQueuedSourceJobTransition = (
  state: InternalControlState,
  job: SourceJob,
  modifiedAt: number,
  ...
  const next = withSourceState({ ...state, queueDepth: Math.max(0, state.queueDepth - 1) }, job.sourceId, (entry) => {
    const { nextDueAt: _nextDueAt, ...entryWithoutDueAt } = entry;
    return {
      ...entryWithoutDueAt,
      lifecycle: lifecycleForIdleSource(entry),
  ...
  return transition(state, next, undefined, modifiedAt);
};
...
export const finishSourceJobTransition = (
  ...
  const nextState = withSourceState(
    {
      ...
      publication: {
        ...state.publication,
        dirtyGeneration,
        requestedGeneration: state.publication.requestedGeneration + (servedProjectionChanged ? 1 : 0),
      },
  ...
  return transition(
    ...
    nextState,
    {
      ...
      needsPublicationWake: servedProjectionChanged || releasedRtkDependency,
      needsRtk,
...
export const finishPublicationJobTransition = (
  ...
  dataTarget: number,
  result: { readonly publishedAt?: string; readonly revision?: string } | undefined,
  finishedAt: number,
): StateTransition<boolean> => {
  ...
  const next = {
    ...
    publication: {
      ...
      ...(result
        ? {
            lastPublishedAt: result.publishedAt ?? toIso(finishedAt),
