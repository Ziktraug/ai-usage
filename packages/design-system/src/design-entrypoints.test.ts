...
test('public design entrypoints keep framework runtimes explicit', async () => {
  ...
  expect(manifest.exports['./solid']).toEqual({ import: './src/solid.ts', types: './src/solid.ts' });
  ...
  expect(await Bun.file(path.join(packageSourceDirectory, 'solid.ts')).text()).toContain("from './components/toggle'");
...
test('Panda scans the aggregate TypeScript, TSX, and Svelte source surface', async () => {
  const pandaConfig = await Bun.file(path.join(packageDirectory, 'panda.config.ts')).text();
  expect(pandaConfig).toContain("include: ['./src/**/*.{ts,tsx,svelte}']");
});
