 ...
 * spawns the exact command the Playwright webServer uses plus `DEBUG=vite:*`,
 * and polls the port until the first HTTP response or the deadline. Output is
 ...
 *
 * Usage: bun --no-env-file tools/stress-web-dev-start.ts [--iterations 15]
...
const parseIterations = (argumentList: readonly string[]): number => {
  if (argumentList.length === 0) {
    return DEFAULT_ITERATIONS;
  }
  const iterations = argumentList[0] === '--iterations' ? Number(argumentList[1]) : Number.NaN;
  if (!(Number.isSafeInteger(iterations) && iterations > 0 && iterations <= 200)) {
    throw new Error('Usage: stress-web-dev-start.ts [--iterations <1-200>]');
  }
  return iterations;
};
...
const runIteration = async (iteration: number): Promise<IterationResult> => {
  ...
  const logFile = path.join(logDirectory, `iteration-${String(iteration).padStart(3, '0')}-${outcome}.log`);
  await writeFile(logFile, `${lines.join('\n')}\n`, 'utf8');
  return { elapsedMs, iteration, outcome };
};
...
for (let iteration = 1; iteration <= iterations; iteration += 1) {
  ...
  results.push(result);
  console.log(`iteration ${result.iteration}/${iterations}: ${result.outcome} in ${result.elapsedMs}ms`);
}
